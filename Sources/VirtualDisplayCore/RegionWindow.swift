...
public final class RegionWindow: NSWindow, NSWindowDelegate {
    ...
    public func snapToWindowBelow() -> Bool {
        ...
        let centre = CGPoint(x: frame.midX, y: primary.frame.height - frame.midY)  // CG space
        ...
        for w in listed {
            guard (w[kCGWindowLayer as String] as? Int) == 0,
                  (w[kCGWindowOwnerPID as String] as? Int32) != mine,
                  let bounds = w[kCGWindowBounds as String] as? [String: Any],
                  let rect = CGRect(dictionaryRepresentation: bounds as CFDictionary),
                  rect.contains(centre)
            else { continue }
            place(Geometry.appKitRect(fromCG: rect, primaryHeight: primary.frame.height))
            return true
        }
        return false
    }
